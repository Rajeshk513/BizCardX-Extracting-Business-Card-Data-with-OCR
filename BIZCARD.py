...
con = psycopg2.connect(host="localhost", user="postgres", password="Rajesh@123", port=5432, database="ocr")
raj = con.cursor()
...
if selected == 'Home':
    ...
    with col1:
        raj.execute('select name from business_cards')
        y = raj.fetchall()
    ...
    if selected_mode == 'View':
        ...
        with col5:
            raj.execute(f"select name, designation, company, email, website, primary_no, secondary_no, "
